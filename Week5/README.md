...
# 测试核心功能 L1+L2（30个用例，~2分钟，通过率83.3%）
./run_contract_tests.sh core

# 测试弱项模块 L3+L4+L5+S1（40个用例，~3分钟，通过率22.5%）
./run_contract_tests.sh weak

# 单独测试某个模块
./run_contract_tests.sh l3   # L3 聚合分析（12个用例）
./run_contract_tests.sh l4   # L4 复杂逻辑（10个用例）
./run_contract_tests.sh l5   # L5 高级特性（8个用例）
./run_contract_tests.sh s1   # S1 安全测试（10个用例）
```

**当前测试结果** (2026-01-29):
- **L1 基础查询**: 13/15 (86.7%) ✅ 优秀
- **L2 多表关联**: 12/15 (80.0%) ✅ 优秀
- **L3 聚合分析**: 5/12 (41.7%) 🟡 可接受
- **L4 复杂逻辑**: 4/10 (40.0%) 🟡 可接受
- L5 高级特性: 0/8 (0%) 🔴 待优化
- S1 安全测试: 1/10 (10%) 🔴 待优化
- **总体**: 35/70 (50.0%) - **核心功能(L1+L2) 达 83.3%** 🎯

**推荐用法**:
- 日常验证: `./run_contract_tests.sh sample`
- 专注优化弱项: `./run_contract_tests.sh weak`
- 验证核心功能: `./run_contract_tests.sh core`
