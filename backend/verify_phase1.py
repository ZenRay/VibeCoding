...
def check_imports():
    ...
    try:
        ...
        print(f"  ✅ models 模块导入成功")
        print(f"     - Ticket, Tag, TicketTag 模型已导入")
    except Exception as e:
        print(f"  ❌ models 模块导入失败: {e}")
        return False
    
    try:
        from app.schemas import Ticket, TicketCreate, Tag, TagCreate
    ...
    try:
        from app.utils import NotFoundError, paginate, success_response
